namespace :bgg do
  desc "BGG 1001-2000位のゲームを登録・更新する（自動翻訳、カテゴリー変換、システムレビューを含む）"
  task register_1001_2000: :environment do
    Rails.logger.info "BGG 1001-2000位のゲーム登録を開始します"
    puts "BGG 1001-2000位のゲーム登録を開始します"
    ...
    # BGGから1001-2000位のゲームIDを取得
    games_1001_2000_ids = get_bgg_1001_2000_game_ids
    
    if games_1001_2000_ids.empty?
      puts "BGGから1001-2000位のゲームIDを取得できませんでした"
      return
    end
    
    puts "取得したゲーム数: #{games_1001_2000_ids.count}件"
    
    # 新規登録が必要なゲームと既存のゲームに分ける
    new_game_ids = []
    existing_games = []
    
    games_1001_2000_ids.each do |bgg_id|
    ...
    if new_game_ids.any?
      ...
      new_game_ids.each_with_index do |bgg_id, index|
        puts "\n--- 新規ゲーム #{index + 1}/#{new_game_ids.count} ---"
        puts "処理対象BGG ID: #{bgg_id}"
        
        begin
          game = register_new_game_with_retry(bgg_id, system_user)
          if game
            total_registered += 1
            puts "  ✓ 新規登録: #{game.name} (BGG ID: #{game.bgg_id})"
          else
            total_errors += 1
            puts "  ✗ 登録失敗: BGG ID #{bgg_id}"
          end
        rescue => e
          total_errors += 1
          puts "  ✗ エラー (BGG ID: #{bgg_id}): #{e.message}"
    ...
    if existing_games.any?
      ...
      existing_games.each_with_index do |existing_game, index|
        begin
          ...
          if update_existing_game_with_retry(existing_game, system_user)
            total_registered += 1
            puts "  ✓ 更新完了"
          else
            total_skipped += 1
    ...
    puts "\n" + "="*60
    puts "BGG 1001-2000位のゲーム登録が完了しました"
    ...
    Rails.logger.info "BGG 1001-2000位のゲーム登録完了: 登録・更新#{total_registered}件, スキップ#{total_skipped}件, エラー#{total_errors}件"
  end
  
  private
  
  # BGG 1001-2000位のゲームIDを取得
  def get_bgg_1001_2000_game_ids
    puts "BGGから1001-2000位のゲームIDを取得中..."
    
    game_ids = []
    
    # BGGランキングページから1001-2000位のゲームIDを取得
    # ページネーションで100件ずつ取得（1001-1100, 1101-1200, ..., 1901-2000）
    (11..20).each do |page|
    ...
    game_ids.uniq.first(1000)  # 確実に1000件以下にする
  end

  # 新規ゲームを登録（リトライ機能付き）
  def register_new_game_with_retry(bgg_id, system_user, max_retries = 3)
    retries = 0
    ...
    begin
      ...
      # メタデータを設定
      set_game_metadata(game, bgg_game_info)
      ...
      retries += 1
      puts "    リトライ #{retries}/#{max_retries}: #{e.message}"
      
      if retries < max_retries
        sleep 10 * retries  # 指数バックオフ
        retry
      else
        puts "    最大リトライ回数に達しました"
        raise e
  ...
  # 既存ゲームを更新（リトライ機能付き）
  def update_existing_game_with_retry(game, system_user, max_retries = 3)
    retries = 0
    ...
    begin
      ...
      retries += 1
      puts "    更新リトライ #{retries}/#{max_retries}: #{e.message}"
  ...
  # ゲームのメタデータを設定
  def set_game_metadata(game, bgg_game_info)
  ...
  def convert_and_store_categories_mechanics(game)
    ...
    # 変換処理
    convert_categories_and_mechanics(game, bgg_category_to_site_category, bgg_category_to_site_mechanic, 
                                   bgg_mechanic_to_site_category, bgg_mechanic_to_site_mechanic,
                                   site_categories, site_mechanics)
  ...
  def convert_categories_and_mechanics(game, cat_to_cat, cat_to_mech, mech_to_cat, mech_to_mech, site_categories, site_mechanics)
    # BGGカテゴリーから当サイトのカテゴリー・メカニクスに変換
    if game.categories.present?
      game.categories.each do |bgg_category|
        if category = cat_to_cat[bgg_category]
          site_categories << category
        elsif mechanic = cat_to_mech[bgg_category]
    ...
    if game.mechanics.present?
      game.mechanics.each do |bgg_mechanic|
        if category = mech_to_cat[bgg_mechanic]
          site_categories << category
        elsif mechanic = mech_to_mech[bgg_mechanic]
          site_mechanics << mechanic
        end
      end
    end
  ...
  def create_system_reviews(game, system_user, bgg_game_info)
    ...
    reviews_to_create.times do |i|
      ...
      # おすすめプレイ人数を設定
      recommended_players = get_recommended_players(game, bgg_game_info)
  ...
  def get_recommended_players(game, bgg_game_info)
    recommended_players = []
    if bgg_game_info[:best_num_players].present?
