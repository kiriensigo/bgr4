...
def fetch_bgg_top_games(start_rank = 1, end_rank = 1000, batch_size = 100)
  ...
  while current_rank <= end_rank
    ...
    # BGGのページは100件ごとに区切られている
    page_num = ((current_rank - 1) / 100) + 1
    ...
    begin
      ...
      # ゲームIDとランクを抽出
      rows = html.css('tr#row_')
      
      batch_ids = []
      rows.each do |row|
        rank_elem = row.css('td.collection_rank').first
        link_elem = row.css('td.collection_objectname a').first
        
        if rank_elem && link_elem && link_elem['href']
          rank = rank_elem.text.strip.to_i
          
          # 指定範囲内のランクのみ処理
          if rank >= current_rank && rank <= end_batch
            href = link_elem['href']
            if href.match(/\/boardgame\/(\d+)\//)
              game_id = $1
              name = link_elem.text.strip
              batch_ids << {id: game_id, rank: rank, name: name}
            end
          end
        end
      end
