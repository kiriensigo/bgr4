...
# 個別に確認
puts "\n📋 対象ゲーム一覧:"
missing_metadata_games.limit(10).each do |game|
  puts "  - #{game.name} (BGG ID: #{game.bgg_id}) - metadata: #{game.metadata.inspect}"
end

if missing_metadata_games.count == 0
  puts "\n🔍 全ゲームのメタデータ状況を確認してみます..."
  ...
  recent_games.each do |game|
    ...
    puts "  metadata present: #{game.metadata.present?}"
    puts "  metadata content: #{game.metadata.inspect}"
    
    if game.metadata.present?
      begin
        parsed = JSON.parse(game.metadata)
        puts "  categories: #{parsed['categories']&.join(', ') || 'なし'}"
        puts "  mechanics: #{parsed['mechanics']&.join(', ') || 'なし'}"
